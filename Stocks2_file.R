...
pacman::p_load(DMwR,ggmap,maptools,maps,bizdays,plotly, readr, dplyr,tidyr,RColorBrewer,imputeTS,lubridate,stringr,d3heatmap,data.table,ggthemes,scales,formattable,viridis,animation,ggmap,leaflet,gridExtra,showtext,extrafont,plyr,stringi)
...
startDate = as.Date("2017-03-15") #Specify period of time we are interested in
endDate = as.character(Sys.Date())

getSymbols(TICKERS[1:50], from = startDate, to = endDate)
getSymbols(TICKERS[51:100], from = startDate, to = endDate)
getSymbols(TICKERS[101:150], from = startDate, to = endDate)
getSymbols(TICKERS[151:200], from = startDate, to = endDate)
getSymbols(TICKERS[201:250], from = startDate, to = endDate)
getSymbols(TICKERS[251:300], from = startDate, to = endDate)
getSymbols(TICKERS[301:350], from = startDate, to = endDate)
getSymbols(TICKERS[351:400], from = startDate, to = endDate)
getSymbols(TICKERS[401:450], from = startDate, to = endDate)
getSymbols(TICKERS[451:length(TICKERS)], from = startDate, to = endDate)
rm(SP500)
#setwd('F:/Predictive Modeling')
#write.csv(MASTER_3_22,file="sent_3_22.csv", row.names = F)
...
cleaning_stocks <- function(stock.xts){
  ...
  stock.df$Ticker <- as.character(colnames(data.frame(date=index(stock.xts), coredata(stock.xts)))[2])
...
rm(cleaned);rm(endDate);rm(i);rm(startDate);rm(STOCKS);rm(TICKERS);rm(cleaning_stocks)
rm(x)
STOCKS<-do.call(rbind, eapply(.GlobalEnv,function(x) data.frame(date=index(x), coredata(x))))
Ticker1<- as.data.frame(str_split(as.character(STOCKS$Ticker), ".", fixed=TRUE))
...
write.csv(STOCKS, file="STOCKS_prices.csv")

Jan4$Dollar.chg <- (Jan4$close-Jan4$open)
Jan4$Pct.chg <- (Jan4$Dollar.chg/Jan4$open)
Jan4$Volatility <- (Jan4$high-Jan4$low)/Jan4$close
Jan4$up.down <- ifelse(Jan4$close/Jan4$open >= 1.01,1,0)
Jan4$up.down.lag <- unlist(tapply(Jan4[,"up.down"],Jan4[,"symbol"], function(x) c(x[-1],NA)))


install.packages("Quandl")
-j9yoEmKLLWhAj29ZtS5
library(Quandl)
Quandl.api_key("-j9yoEmKLLWhAj29ZtS5")
...
mydata2 = Quandl.datatable("ZACKS/FC", ticker="AAPL", paginate=T)
